...
MessageHandle.heartbeat = function (owner, pid, data) {
  owner.pstatus[pid] = _extend(_extend(owner.pstatus[pid], msg.data), {
    '_time' : Date.now()
  });
};
...
MessageHandle.broadcast = function (owner, pid, data) {
  ...
  owner.emit('broadcast', pid, data.who, data.msg);
};
  ...
  Child.prototype._fork = function () {
    ...
    sub.on('exit', function (code, signal) {
      ...
      if (code || 'SIGKILL' === signal) {
        ...
        if (n >= _options.max_fatal_restart && _self.pfatals[n - 1] + 60000 >= t) {
          _self.emit('giveup', n);
          setTimeout(_self.start, 60100);
          return;
        }
      }
      _self.start();
