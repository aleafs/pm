/* vim: set expandtab tabstop=2 shiftwidth=2 foldmethod=marker: */

"use strict";

var net = require('net');
var util = require('util');
var Emitter = require('events').EventEmitter;

exports.create = function (options, PROCESS) {

  /**
   * @ 配置参数
   */
  var _options  = {
    'heartbeat_interval' : 2000,
    'terminate_timeout'  : 1000,
  };
  for (var i in options) {
    _options[i] = ~~Number(options[i]);
  }

  /**
   * @ 方便单元测试
   */
  PROCESS = PROCESS || process;

  /**
   * @ send message to master
   */
  var askmaster = function () {};
  if ('function' === (typeof PROCESS.send)) {
    askmaster = function (type, data) {
      try {
        PROCESS.send({'type' : type, 'data' : data});
      } catch (e) {
      }
    }
  }

  /**
   * @ 连接处理
   */
  var onconnect = function (client, which) {
    client.end();
  };

  /**
   * @ 监听句柄
   */
  var _listener = {};

  /**
   * @ 记分板
   */
  var _scores = {};
  var _listenAt = function (idx, handle, m) {
    _listener[idx] = net.createServer(function (c) {
      if (!_scores[idx]) {
        _scores[idx] = 1;
      } else {
        _scores[idx]++;
      }
      onconnect(c, idx);
    }).listen(handle, function () {
      m.emit('listen', idx);
    });
  };

  /**
   * @ 心跳函数
   */
  var heartbeat = function () {
    askmaster('heartbeat', {
      'hbterm' : _options.heartbeat_interval,
      'scores' : _scores,
      'memory' : PROCESS.memoryUsage(),
    });
  };
  PROCESS.send && setInterval(heartbeat, _options.heartbeat_interval);

  /* {{{ private function suicide() */
  var suicide = function () {
    Object.keys(_listener).forEach(function (i) {
      try {
        _listener[i].close();
        delete _listener[i];
      } catch (e) {
      }
    });
    setTimeout(function () {
      PROCESS.exit(0);
    }, _options.terminate_timeout);
  };
  /* }}} */

  /* {{{ public Worker constructor */
  var Worker = function () {
    Emitter.call(this);

    var _self = this;
    PROCESS.on('message', function (msg, handle) {
      if (!msg || !msg.type) {
        return;
      }

      if (handle && 'listen' === msg.type) {
        var idx = msg.data;
        if (!_listener[idx]) {
          _listenAt(idx, handle, _self);
        } else {
          try {
            _listener[idx].close(function () {
              _listenAt(idx, handle, _self);
            });
          } catch (e) {
          }
        }
        return;
      }

      switch (msg.type) {
        case 'suicide':
          suicide();
          _self.emit('suicide', 'message');
          break;

        case 'hello':
          _self.emit('message', msg.data, msg.from, msg._pid);
          break;

        default:
          break;
      }
    });
  };
  util.inherits(Worker, Emitter);
  /* }}} */

  /* {{{ public prototype ready() */
  Worker.prototype.ready = function (callback) {
    /**
     * @ 找master要监听handle
     */
    askmaster('gethandle');
    if ('function' === (typeof callback)) {
      onconnect = callback;
    };
  };
  /* }}} */

  /* {{{ public prototype broadcast() */
  /**
   * Broadcast message around workers
   */
  Worker.prototype.broadcast = function (who, msg) {
    askmaster('broadcast', {
      'who' : who,
      'msg' : msg,
    });
  };
  /* }}} */

  var _me = new Worker();

  PROCESS.on('SIGHUB',  function () {});
  PROCESS.on('SIGTERM', function () {
    suicide();
    _me.emit('suicide', 'SIGTERM');
  });
  PROCESS.once('exit', function () {
    _me.emit('exit');
  });

  return _me;
};

