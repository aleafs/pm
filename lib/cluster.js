  ...
  sub.on('exit', function (code, signal) {
    ...
    if (code && 'SIGKILL' != signal) {
        ...
        if (obj.restarts[name].length > (obj.options.max_fatal_restart * obj.bindings[name].cnum)) {
            return callEvent(
                obj, 'restartgiveup', name, 
                obj.restarts[name].length+' times fatal in '+obj.options.restart_time_window+' seconds.'
            );
        }
    }
 ...
 * @param {Function} callback function, with arguments like:
...
Master.prototype.on = function(evt, callback) {
  ...
  for (var idx in evt) {
    if ('function' == (typeof evt[idx])) {
      this.__events[idx] = evt[idx];
    }
  }
