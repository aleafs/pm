 ...
 * @api public
 */
Master.prototype.register = function(name, path, cnum, config) {
  if ('number' != (typeof cnum)) {
    cnum = require('os').cpus().length;
  }
  ...
  this.bindings[name]  = {
    ...
    'cnum'  : intval(cnum, 1),
    'pnum'  : 0,
    'gone'  : (config && config.gone) ? new Buffer(config.gone) : RESPONSE_ON_NO_WORKER,
    'maxc'  : (config && config.user) ? intval(config.max_conn, -1) : -1,
  };
  this.heartmsg[name]  = {};
  this.killings[name]  = {};

  if (config && config.user) {
	RUNUSER[name]	= config.user;
  }
  return this;
};
