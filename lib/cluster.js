 ...
 * @注册子进程映射表
 */
Master.register	= function (port, path, cnum) {
...
Master.mainloop	= function () {
	for (var port in Master.bindings) {
		...
		var has	= Master.children[port] ? Master.children[port] : {};
		for (var pid in has) {
		...
		cnt < 1 && port > 0 && Master.listen(port);
		while (cnt < cfg.cnum) {
			Master.startup(cfg.path, port);
			cnt++;
		}
	}
};
 ...
 * @启动子进程
 */
Master.startup = function (path, port) {
