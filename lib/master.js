...
var createWorker = function(argv, options, name, master) {
  ...
  var _fork = function () {
    ...
    NOTICE('new worker forked (' + pid + ') as "' + argv.join(' ') + '"');
...
_Master.prototype.register = function (name, file, options, argv) {
  ...
  __WORKERS_LIST[name] = createWorker(argv, options, name);
};
