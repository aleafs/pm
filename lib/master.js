...
var _workerPair = function (argv, options, name) {
      ...
      switch (msg.type) {
        case MESSAGE.STATUS:
          ...
          if (pstat && pstat._time) {
            _hbtime = now - pstat._time;
            if (_hbtime > MAX_HEARTBEAT_INTERVAL) {
              _hbtime = MAX_HEARTBEAT_INTERVAL;
            }
          }
  ...
  _options.max_heartbeat_lost && setInterval(function () {
    var die = Date.now();
    if (_options.max_heartbeat_lost < 0) {
      die -= (2 * MAX_HEARTBEAT_INTERVAL);
    } else {
      die -= (_options.max_heartbeat_lost * _hbtime);
    }
    for (var pid in _pstatus) {
      var pstat = _pstatus[pid];
      if (pstat.uptime < die && (!pstat._time || pstat._time < die)) {
  ...
  _me.reload = function () {
    // _newChild 会修改 _pstatus, 所以先取出所有keys
    var pids = Object.keys(_pstatus);
